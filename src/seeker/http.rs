...
impl Seeker for HttpSeeker {
    ...
    fn read<'a>(
        ...
        async move { self.internal_read_impl(buf).await }
    }
    ...
    fn seek(
        ...
        async move { self.internal_seek_impl(pos).await }
    }
...
impl HttpSeeker {
    async fn internal_read_impl(&mut self, buf: &mut [u8]) -> Result<usize, IoError> {
        if buf.is_empty() {
            return Ok(0);
        }
    ...
    async fn internal_seek_impl(&mut self, pos: SeekFrom) -> Result<u64, IoError> {
